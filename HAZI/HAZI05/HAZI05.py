...
class KNNClassifier:
    ...
    def train_test_slpit(self, features: pd.DataFrame, labels: pd.Series) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
    ...
    def best_k(self) -> Tuple[int, float]:
        ...
        for k in range(1,21):
            ...
            accs.append((k,accuracy))
        best_k = max(accs, key=lambda x:x[1])
