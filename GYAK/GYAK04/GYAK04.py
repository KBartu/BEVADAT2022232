...
# %%
def get_column(test_df: pd.DataFrame, area:str):
    new_df = test_df.copy()
    return new_df[area]
...
def population_density(test_df: pd.DataFrame):
    new_df = test_df.copy()
    d = new_df["population"] / new_df["area"]
    new_df["density"] = d
    return new_df
...
def plot_population(test_df: pd.DataFrame) -> plt.figure:
    new_df = test_df.copy()
    fig, a = plt.subplots()
    a.bar(new_df["country"], new_df["population"])
    a.set_xlabel("Country")
    a.set_ylabel("Population (millions)")
    a.set_title("Population of Countries")
    return fig
...
def plot_area(test_df: pd.DataFrame) -> plt.figure:
    new_df = test_df.copy()
    fig, a = plt.subplots()
    a.set_title("Area of Countries")
    a.pie(new_df["area"], labels=new_df["country"])
    return fig
